    ...
    for dataset, name_prefix in [(trainset, "train"), (devset, "dev")]:
        with open(
            ...
            for i, datapoint in enumerate(dataset):
                ...
                np.save(
                    os.path.join(
                        FLAGS.output_directory, "mels", f"{name_prefix}_output_{i}.npy"
                    ),
                    spec,
                )
                ...
                if r != 16000:
                    audio = librosa.resample(
                        audio, orig_sr=r, target_sr=16000, res_type="kaiser_fast"
                    )
                audio = np.clip(
                    audio, -1, 1
                ...
                sf.write(
                    os.path.join(
                        FLAGS.output_directory, "wavs", f"{name_prefix}_output_{i}.wav"
                    ),
                    audio,
                    16000,
                )
